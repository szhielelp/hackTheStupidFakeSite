...
(function (global, factory) {
  ...
  const GBK = function (gbk_us) {
    ...
    const gbk = {
      ...
      encode(str) {
        str += '';
        const gbk = [];
        const wh = '?'.charCodeAt(0); // gbk中没有的字符的替换符
        for (let i = 0; i < str.length; i++) {
          const charcode = str.charCodeAt(i);
          if (charcode < 0x80) gbk.push(charcode);
          else {
            let gcode = gbk_us.indexOf(charcode);
            if (~gcode) {
              gcode += arr_index;
  ...
  const URI = function (GBK) {
    const passChars = '!\'()*-._~';
    const otherPassChars = '#$&+,/:;=?@';
    function getModue(passChars) {
      const passBits = passChars.split('').sort();
      const isPass = function (s) {
      ...
      return {
        ...
        decode(enstr) {
          return enstr.replace(/(%[\dA-Za-z]{2})+/g, (a, b, c) => {
            let str = '';
            const arr = a.match(/.../g);
            for (let i = 0; i < arr.length; i++) {
              const hex = arr[i];
              const code = parseInt(hex.substr(1), 16);
              if (code & 0x80) {
    ...
    const URIComponent = getModue(passChars);
    const URI = getModue(passChars + otherPassChars);
  ...
  const src = function (gbk_us) {
	    const gbk$$1 = gbk(gbk_us);
	    gbk$$1.URI = URI(gbk$$1);
	    return gbk$$1;
  };

  // 多进制转换后的数字还原函数 构建时会替换占位符
  const Fn_Hex_decode = function decode() {
    let n = 0;
    const str = arguments[0];
    for (let i = 0, w = str.length; i < w; i++) {
  ...
  const Fn_unzip = function unZip() {
    return arguments[0].replace(/\x23(\d+)\x24/g, (a, b) => Array(+b + 4).join('#'))
      .replace(/[\x26-\x7e]\x25[\x26-\x7e]/g, (a) => {
        let b = a.substr(0, 1).charCodeAt(0);
        const e = a.substr(2).charCodeAt(0);
      ...
      .replace(/\x23/g, '###')
      .replace(/([\x26-\x7e]{2})\x21([\x26-\x7e\x23]+)(?:\x20|$)/g, (all, hd, dt) => dt.replace(/./g, (a) => {
        if (a != '#') {
          return hd + a;
        }
        return a;
  ...
  function gbkArray(gbkArr) {
    ...
    for (let i = 0x81, k = 0; i <= 0xfe; i++) {
      ...
      for (let j = 0x40; j <= 0xfe; j++) {
        if (
          (j == 0x7f)
					|| ((i >= 0xa1 && i <= 0xa7) && j <= 0xa0)
					|| ((i >= 0xaa && i <= 0xaf) && j >= 0xa1)
					|| (i >= 0xf8 && j >= 0xa1)
        ) {
          data.push(undefined);
          continue;
